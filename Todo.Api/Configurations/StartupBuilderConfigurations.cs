...
public static class StartupBuilderConfigurations
{
    ...
    /// </param>
    /// <exception cref="InvalidOperationException">
    ///     Thrown when the Redis connection string is not found in the configuration.
    /// </exception>
    public static void AddRedisConnectionString(this WebApplicationBuilder builder)
    {
        var connectionString = builder.Configuration.GetConnectionString(Constants.RedisConnectionStringName)
                                   ?? throw new InvalidOperationException(
                                       $"Redis connection string '{Constants.RedisConnectionStringName}' not found.");
